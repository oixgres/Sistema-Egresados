...
class AnswersToDatabase{
    constructor(totalQuestions) {
        ...
        this.ajaxRequestForSaveAnswer = function(idPregunta, idEmpleador, respuesta){
            $.ajax({
                url: '../php/setEmployerAnswer.php',
                async: false,
                data: {idPregunta, idEmpleador, respuesta},
                type: 'POST',
                success: function (response) {
                    console.log(response)
                    if(parseInt(response) > 0){
                        console.log("SAVE ANSWER NICE")
                        flag = true;

                    }else{
                        console.log("SAVE ANSWER NO NICE")
                        flag = false;
...
$(document).ready(function (e) {
    ...
    const idUsuario = localStorage.getItem("USER_ID")//getCookie(id); //obtener el id de usuario
    ...
    $.ajax({
        url: '../php/getEmployerSurveys.php', //obtener las encuestas
        data:   {}, //mandar el idUsuario
        type:   'POST',
        success: function (response) {
            ...
            try{
                ...
                surveys.forEach(survey => { //para cada encuesta encontrada
                    ...
                    node = $(`
                        <a class="dropdown-item" id="${survey.idEncuesta}">${survey.Nombre}</a>
                    `);
    ...
    $('#Registrar_empleador').on('click', function (e) {
        e.stopPropagation();
        let Nombre = $('#Nombre_usuario').val();
        let Empresa = $('#Nombre_Empresa').val();

        $.ajax({
            url: '../php/createEmployer.php',
            data: {Nombre, Empresa},
            type: 'POST',
            success: function (response) {
                // const idUsuario = localStorage.getItem("USER_ID")//getCookie(id); //obtener el id de usuario
